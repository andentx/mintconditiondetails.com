    ...
    <>
      <div className="mx-auto grid w-full max-w-2xl grid-cols-1 items-start gap-x-8 gap-y-16 py-16 sm:py-16 lg:max-w-full lg:grid-cols-2 ">
        <div>
          ...
          <dl className="grid grid-cols-1 gap-x-6 gap-y-10 pt-4 sm:grid-cols-2 sm:gap-y-16 lg:gap-x-8">
            {menuItems.map((item) => {
              return (
                <div
                  key={item._id}
                ...
                >
                  <dt className="text-2xl font-medium text-gray-900">{item.title}</dt>
                  <dd className="mb-2 mt-2 text-sm text-gray-500 ">
                    <span className="portableText">
                      <PortableText value={item.description} />
                    </span>
                  </dd>
                  <dd className="text mt-2 text-lg font-medium text-gray-900 ">{item.price}</dd>
                </div>
              );
            })}
          </dl>
